...
class BarDynamixelCommander:
	def __init__(self, id):
		self.id = id
		self.dynamixel_com = rospy.ServiceProxy('dynamixel_workbench_bar/dynamixel_command', DynamixelCommand)
...
class BarCommander:
	def __init__(self, bar_dynamixel_id):
		#### variable ####
		# self.floor_switch = False
		# self.vacuum_state = False
		# self.interrupt_flag = False
		...
		#### dynamixel ####
		self.bardynamixelcommander = BarDynamixelCommander(bar_dynamixel_id)
	...
	def setThetaPos(self, pos):
		...
		self.current_theta = pos
		self.bardynamixelcommander.setPosition(pos)

	def setThetaVel(self, vel):
		self.bardynamixelcommander.setVelocity(vel)
